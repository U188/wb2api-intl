...
func TestDefault1MContextDefaultAndExplicitFalse(t *testing.T) {
	if !Default().Features.Default1MContext {
		t.Fatal("Default().features.default_1m_context must be true")
	}
	legacy, err := ParseConfig([]byte(`{"listen":":9999"}`))
	if err != nil {
		t.Fatal(err)
	}
	if !legacy.Features.Default1MContext {
		t.Fatal("legacy config without features.default_1m_context must retain true")
	}
	disabled, err := ParseConfig([]byte(`{"features":{"default_1m_context":false}}`))
	if err != nil {
		t.Fatal(err)
	}
	if disabled.Features.Default1MContext {
		t.Fatal("explicit features.default_1m_context=false was not retained")
	}
}

func TestDefault1MContextEnvOverride(t *testing.T) {
	t.Setenv("WB2A_DEFAULT_1M_CONTEXT", "false")
	c, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
	if c.Features.Default1MContext {
		t.Fatal("WB2A_DEFAULT_1M_CONTEXT=false did not override default")
	}
}

func TestSaveConfigStoresDefault1MContextLive(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.json")
	if err := os.WriteFile(path, []byte(`{"features":{"default_1m_context":true}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := saveConfig([]byte(`{"features":{"default_1m_context":false}}`), path, live, p, up, sch); err != nil {
		t.Fatal(err)
	}
	if live.Load().Default1MContext {
		t.Fatal("saveConfig did not hot-store false")
	}
	if _, err := saveConfig([]byte(`{"features":{"default_1m_context":true}}`), path, live, p, up, sch); err != nil {
		t.Fatal(err)
	}
	if !live.Load().Default1MContext {
		t.Fatal("saveConfig did not hot-store true")
	}
}
