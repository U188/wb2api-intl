...
func fetchUserResource(af *authFile) (remain, used, size int64, packs int, err error) {
	...
	base := billingBaseCN
	site := strings.ToLower(strings.TrimSpace(af.Site))
	domain := strings.ToLower(af.Auth.Domain)
	if site == "intl" || site == "international" || site == "global" ||
		(site == "" && strings.Contains(domain, "codebuddy.ai")) {
		base = billingBaseIntl
	}
	req, err := http.NewRequest(http.MethodPost, base+"/v2/billing/meter/get-user-resource", bytes.NewReader(body))
	if err != nil {
		return 0, 0, 0, 0, err
	}
