...
func TestChatStickyFollowsFinalSuccess(t *testing.T) {
	...
	// 绑定必须收敛到最终成功的 good。
	if uid, ok := st.lastUID("cn:conv-1"); !ok || uid != "good" {
...
func TestModelsDynamic(t *testing.T) {
	...
	up := newFakeUpstream(t, func(authz string) (int, string, bool) {
		return 200, `{"code":0,"data":{"models":[{"id":"dyn-model-a","maxInputTokens":65536,"maxOutputTokens":8192,"maxAllowedSize":64000,"contextWindow":{"supportedLengths":[65536,32000],"defaultLength":32000},"reasoning":{"effort":"medium","supportedEfforts":["low","medium","high"]}},{"id":"dyn-model-b","maxInputTokens":131072,"maxOutputTokens":16384},{"id":"glm-9.9","maxInputTokens":262144,"maxOutputTokens":32768}],"agents":[{"name":"cli","models":["dyn-model-a","dyn-model-b","glm-9.9"]}]}}`, false
	})
	p := testPoolWith(&auth.Auth{UID: "u1", AccessToken: "at1", ExpiresAt: 9999999999})
	h := NewHandler(Config{Pool: p, Upstream: up})
	rec := httptest.NewRecorder()
	...
	for _, m := range data {
		...
		case "dyn-model-a":
			...
			if mm["max_input_tokens"].(float64) != 65536 || mm["max_allowed_size"].(float64) != 64000 {
				t.Errorf("dyn-model-a physical fields: max_input=%v max_allowed=%v", mm["max_input_tokens"], mm["max_allowed_size"])
			}
			cw := mm["context_window"].(map[string]any)
			lengths := cw["supported_lengths"].([]any)
			if len(lengths) != 2 || lengths[0].(float64) != 32000 || cw["default_length"].(float64) != 32000 {
				t.Errorf("dyn-model-a context_window=%v", cw)
			}
			camel := mm["contextWindow"].(map[string]any)
			if camel["defaultLength"].(float64) != 32000 {
				t.Errorf("dyn-model-a contextWindow=%v", camel)
			}
...
func TestModelsDynamicOneMDefaultRequiresPhysicalCapacity(t *testing.T) {
	infos := []upstream.ModelInfo{
		{ID: "physical-1m", ContextWindow: 1_000_000, ContextLengths: []int64{200_000, 1_000_000}, DefaultContextLength: 200_000},
		{ID: "physical-small", ContextWindow: 500_000, ContextLengths: []int64{200_000, 1_000_000}, DefaultContextLength: 200_000},
	}
	got := modelInfoList(infos, auth.SiteCN, true)
	for _, model := range got {
		window := model["context_window"].(map[string]any)
		want := int64(200_000)
		if model["id"] == "physical-1m" {
			want = 1_000_000
		}
		if window["default_length"] != want {
			t.Errorf("%s default=%v want=%d", model["id"], window["default_length"], want)
		}
	}
	live := livecfg.New(livecfg.Snapshot{Default1MContext: false})
	h := NewHandler(Config{Pool: pool.New(""), Live: live})
	if h.loadLive().Default1MContext {
		t.Fatal("live false must remain explicit false")
	}
}
