...
const cnGamificationOnly = "任务与签到仅支持国内账号"
...
// accountCNByUID 是所有单账号 gamification 入口的统一站点闸门。
func (p *Panel) accountCNByUID(w http.ResponseWriter, uid string) *auth.Auth {
	a := p.accountByUID(w, uid)
	if a == nil {
		return nil
	}
	if auth.SiteFrom(a.Site) != auth.SiteCN {
		writeErr(w, http.StatusUnprocessableEntity, cnGamificationOnly)
		return nil
	}
	return a
}
