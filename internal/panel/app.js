...
function go(v) {
  ...
  if (v === 'models' && !$('mdBody-cn').children.length) { loadModels('cn'); loadModels('intl'); }
...
function renderAccounts(list) {
  ...
  tb.innerHTML = list.map(s => {
    ...
    const siteTag = s.site === 'intl' ? '<span class="tag" style="color:#8b5cf6;border-color:#c4b5fd;background:#f5f3ff">国际</span>' : '<span class="tag" style="color:#0f766e;border-color:#99f6e4;background:#f0fdfa">国内</span>';
    const site = s.site === 'intl' ? 'intl' : 'cn';
    ...
    return '<tr class="' + cls + '" title="uid: ' + esc(s.uid) + '">' +
      ...
      '<td class="who"><div class="nm">' + siteTag + ' ' + (s.nickname ? esc(s.nickname) : '<span style="color:var(--ink-3)">未命名</span>') + '</div><div class="id">' + esc(short) + '</div></td>' +
      '<td>' + tag + note + '</td>' +
      ...
      '<td class="acts">' +
        (site === 'intl' ? '' : '<button class="xs ghost" data-a="checkin" data-u="' + esc(s.uid) + '">签到</button>') +
        '<button class="xs ghost" data-a="balance" data-u="' + esc(s.uid) + '">余额</button>' +
        (site === 'intl' ? '' : '<button class="xs ghost" data-a="tasks" data-u="' + esc(s.uid) + '">任务</button>') +
...
/* ── 模型（站点严格隔离，任一站失败只显示在自己的区块）────────────── */
function renderModels(site, d) {
  const tb = $('mdBody-' + site), list = d.models || [];
  if (!list.length) { tb.innerHTML = '<tr><td colspan="8"><div class="empty">该站点上游未返回模型</div></td></tr>'; return; }
  tb.innerHTML = list.map(m => {
    ...
    const lengths = cw.supported_lengths || [];
    const defaultLength = cw.default_length;
    const contextOptions = lengths.length
      ? '<div class="ctx-levels">' + lengths.map(n => '<span class="tag mute">' + formatTokenCount(n) + '</span>').join(' ') +
        '<div class="hint">默认 ' + formatTokenCount(defaultLength) + (lengths.length > 1 ? ' / 可选 ' + lengths.filter(n => n !== defaultLength).map(formatTokenCount).join('、') : '') + '</div></div>'
    ...
    return '<tr><td class="mark" aria-hidden="true"><i></i></td><td class="who"><div class="nm">' + esc(m.id) + '</div><div class="id">' + esc(m.name || '') + '</div></td>' +
      ...
      '<td class="num">' + formatTokenCount(m.context_length) + '</td>' +
      '<td class="num">' + formatTokenCount(m.max_output_tokens) + '</td></tr>';
  }).join('');
  $('mdNote-' + site).textContent = list.length + ' 个模型 · ' + (d.source === 'catalog' ? '官方国际 CLI 目录' : '国内上游动态目录');
}
async function loadModels(site) {
  const tb = $('mdBody-' + site);
  tb.innerHTML = '<tr><td colspan="8"><div class="empty">' + (site === 'intl' ? '正在读取官方国际 CLI 目录…' : '正在向国内上游查询…') + '</div></td></tr>';
  try {
    const d = await api('models?site=' + site);
    if (d.site !== site) throw new Error('站点响应不匹配：期望 ' + site + '，得到 ' + (d.site || '空'));
    renderModels(site, d);
  } catch (e) { tb.innerHTML = '<tr><td colspan="8"><div class="empty">' + esc(e.message) + '</div></td></tr>'; }
}
$('btnModels-cn').onclick = () => loadModels('cn');
$('btnModels-intl').onclick = () => loadModels('intl');
...
const CFG_MAP = {
  ...
  default_1m_context: ['features', 'default_1m_context'],
...
/* ── 添加账号 ─────────────────────────────────────────────────────── */
function openAdd(site = loginSite) {
  loginSite = site;
  ...
  stopPoll();
  api('login/start?site=' + encodeURIComponent(loginSite), { method: 'POST' }).then(r => {
    loginState = r.state;
...
/* ── 顶部动作 ─────────────────────────────────────────────────────── */
$('btnAdd').onclick = () => openAdd('cn');
$('btnSiteCN').onclick = () => openAdd('cn');
$('btnSiteIntl').onclick = () => openAdd('intl');
