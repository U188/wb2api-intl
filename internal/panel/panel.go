...
type Config struct {
	...
	// Live 运行期可变配置（在线改配置立即生效）；nil 时 Default1MContext 提供静态回退。
	Live             *livecfg.Holder
...
type Panel struct {
	...
	// logins 进行中的 OAuth 设备授权会话（state → 会话信息：创建时刻 + 站点）。
...
// default1MContext 返回当前模型元数据默认预算开关（Live 优先）。
func (p *Panel) default1MContext() bool {
	if p.cfg.Live != nil {
		return p.cfg.Live.Load().Default1MContext
	}
	return p.cfg.Default1MContext
}
...
// models 查询模型列表与 reasoning 档位。CN 使用现有动态接口；Intl 使用官方
// 国际 CLI 内置目录（不要求账号，也绝不请求 /console 接口）。
func (p *Panel) models(w http.ResponseWriter, r *http.Request) {
	site := r.URL.Query().Get("site")
	if site != auth.SiteCN && site != auth.SiteIntl {
		writeErr(w, http.StatusUnprocessableEntity, "site must be cn or intl")
		return
	}
	var (
	...
	if site == auth.SiteIntl {
		...
		if p.cfg.Upstream != nil {
			infos, err = p.cfg.Upstream.FetchModels(&auth.Auth{Site: auth.SiteIntl})
		} else {
			infos = upstream.IntlBuiltinModels()
		}
	} else {
		acct := p.cfg.Pool.PickForSite(site)
		if acct == nil {
			writeErr(w, http.StatusServiceUnavailable, "指定站点没有可用账号")
			return
		}
	...
	default1M := p.default1MContext()
	out := make([]map[string]any, 0, len(infos))
	for _, mi := range infos {
		defaultLength := upstream.EffectiveDefaultContextLength(mi.ContextLengths, mi.DefaultContextLength, mi.ContextWindow, default1M)
		contextWindow := map[string]any{
			"supported_lengths": mi.ContextLengths,
		...
		out = append(out, map[string]any{
			...
			"max_input_tokens":     mi.ContextWindow,
			"max_output_tokens":    mi.MaxTokens,
			"max_allowed_size":     mi.MaxAllowedSize,
			"context_window":       contextWindow,
			"contextWindow":        map[string]any{"supportedLengths": mi.ContextLengths, "defaultLength": defaultLength},
	...
	body := map[string]any{"ok": true, "site": site, "models": out}
	if source != "" {
		body["source"] = source
		body["hint"] = "官方国际 CLI 目录"
	}
