...
func (p *Panel) tasksScanAll(w http.ResponseWriter, r *http.Request) {
	...
	items := make([]scanAccountItem, 0, len(states))
	var wg sync.WaitGroup
	var mu sync.Mutex
	for _, st := range states {
		if st.Disabled || auth.SiteFrom(st.Site) != auth.SiteCN {
			continue
		}
		wg.Add(1)
		go func(uid string) {
			...
			it := scanAccountItem{UID: uid, Nickname: a.Nickname}
...
func (p *Panel) runQueueItems(accts []queueAccount, items []queueItem, concurrency int) {
	...
	for _, one := range accts {
		...
		go func(one queueAccount) {
			defer wg.Done()
			if one.a == nil || auth.SiteFrom(one.a.Site) != auth.SiteCN {
				if one.a != nil {
					p.queueSet(q, one.a.UID, func(it *queueItem) {
						it.Status, it.Message = "skipped", cnGamificationOnly
					})
				}
				return
			}
...
func (p *Panel) runGrowthQueued(a *auth.Auth, code string) (string, error) {
	if a == nil || auth.SiteFrom(a.Site) != auth.SiteCN {
		return "", fmt.Errorf("%s", cnGamificationOnly)
	}
