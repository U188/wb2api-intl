...
type Config struct {
	...
	// AvailableForModel 保留旧的模型感知全池回调。
	AvailableForModel func(model string) []string
	// AvailableForSiteModel 为路径路由提供站点+模型严格候选集。
	AvailableForSiteModel func(site, model string) []string
}
...
// Resolve 返回无站点、无模型维度的绑定。
func (r *Router) Resolve(key string) (string, bool) {
	return r.ResolveForSiteModel(key, "", "")
}

// ResolveForModel 保留旧的全池模型感知语义。
func (r *Router) ResolveForModel(key, model string) (string, bool) {
	return r.ResolveForSiteModel(key, "", model)
}

// ResolveForSiteModel 返回会话 key 在指定站点和模型上的绑定。
func (r *Router) ResolveForSiteModel(key, site, model string) (string, bool) {
	now := time.Now()
	available := r.availableSet(site, model)
	...
	uids := r.availableSlice(site, model)
	if len(uids) == 0 {
		return "", false
	}
...
// availableSet 把站点+模型候选列表转集合（快路径绑定校验用）。
func (r *Router) availableSet(site, model string) map[string]bool {
	uids := r.availableSlice(site, model)
...
// availableSlice 优先使用站点感知回调，旧调用则兼容模型/全池回调。
func (r *Router) availableSlice(site, model string) []string {
	if r.cfg.AvailableForSiteModel != nil {
		return r.cfg.AvailableForSiteModel(site, model)
	}
