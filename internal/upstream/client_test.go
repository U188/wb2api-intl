...
func TestFetchModelsContextWindowExplicitAndDeepSeekBackfill(t *testing.T) {
	c := testClient(func(r *http.Request) (*http.Response, error) {
		return jsonResp(200, `{"code":0,"data":{"models":[
			{"id":"other","maxInputTokens":500000,"contextWindow":{"supportedLengths":[200000,-1,100000,200000,900000,1.5,"bad",null],"defaultLength":200000}},
			{"id":"DeepSeek-V4.1-Pro-ioa","maxInputTokens":1000000,"maxOutputTokens":12345},
			{"id":"deepseek-v4-flash-restricted","maxInputTokens":128000,"maxAllowedSize":128000} 
		],"agents":[{"name":"cli","models":["other","DeepSeek-V4.1-Pro-ioa","deepseek-v4-flash-restricted"]}]}}`), nil
	})
	...
	other := byID["other"]
	if !reflect.DeepEqual(other.ContextLengths, []int64{100000, 200000}) || other.DefaultContextLength != 200000 {
		t.Fatalf("explicit contextWindow = %v default=%d", other.ContextLengths, other.DefaultContextLength)
	}
	ds := byID["DeepSeek-V4.1-Pro-ioa"]
	if ds.ContextWindow != 1000000 || ds.MaxAllowedSize != 1000000 || ds.MaxTokens != 12345 {
		t.Fatalf("deepseek physical limits: %#v", ds)
	}
	if !reflect.DeepEqual(ds.ContextLengths, []int64{200000, 1000000}) || ds.DefaultContextLength != 200000 {
		t.Fatalf("deepseek derived canonical contextWindow = %v default=%d", ds.ContextLengths, ds.DefaultContextLength)
	}
	restricted := byID["deepseek-v4-flash-restricted"]
	if restricted.ContextWindow != 128000 || restricted.MaxAllowedSize != 128000 || len(restricted.ContextLengths) != 0 {
		t.Fatalf("explicit restricted limits must win: %#v", restricted)
	}
}
