...
// （官方客户端 matches 规则，对齐官方 apply 门控）。
//
...
func backfillReasoningContent(obj map[string]any) {
	...
	// thinkingEnabled 半边：读注入后的 thinking.type（与官方 thinkingEnabled 对应）。
	...
	// reasoning 字段存在且非空（部分账号/租户对 thinking 形态校验 len(reasoning)>0：
	// 缺失/null/空串 400，空白串 200；官方 CLI 本就给 assistant 挂上一轮 reasoning 文本）。
