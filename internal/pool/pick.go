...
func (p *Pool) Pick() *auth.Auth {
	return p.PickForSite("")
}

// PickForSite 站点感知选号：site 非空时只在指定站点账号池内选择；空 = 不限。
// 老凭证的空/未知 site 由 auth.SiteFrom 归一为 cn。
func (p *Pool) PickForSite(site string) *auth.Auth {
	return p.pick(nil, "", site)
}
...
func (p *Pool) PickExcludingForModel(tried map[string]bool, reqModel string) *auth.Auth {
	return p.PickForSiteModel(tried, reqModel, "")
}

// PickForSiteModel 站点+模型感知选号：site 非空限定站点池，reqModel 非空启用
// 6004 模型豁免，二者可独立生效。
func (p *Pool) PickForSiteModel(tried map[string]bool, reqModel, site string) *auth.Auth {
	return p.pick(tried, reqModel, site)
}
...
func (p *Pool) pick(tried map[string]bool, reqModel, site string) *auth.Auth {
	p.mu.Lock()
	defer p.mu.Unlock()
	now := time.Now()
	...
	if site != "" {
		// 站点隔离是路径路由的硬边界；任何 normal/fallback 候选都必须同站。
		base := healthyOf
		healthyOf = func(e *entry) bool {
			return base(e) && e.a != nil && auth.SiteFrom(e.a.Site) == site
		}
	}
	var cands []*entry
	...
	if len(cands) == 0 {
		...
		// site 非空时兜底同样限定站点（避免跨站选号）。
		if site != "" {
			return p.pickEarliestExpiryForSiteLocked(tried, now, site)
		}
...
func (p *Pool) pickEarliestExpiryLocked(tried map[string]bool, now time.Time) *auth.Auth {
	return p.pickEarliestExpiryForSiteLocked(tried, now, "")
}

// pickEarliestExpiryForSiteLocked 站点感知兜底：site 非空时只在指定站点的冷却账号中
// 选截止最早的一个（跨站隔离），空 = 不限（原语义）。
func (p *Pool) pickEarliestExpiryForSiteLocked(tried map[string]bool, now time.Time, site string) *auth.Auth {
	var best *entry
	for uid, e := range p.byUID {
		if tried != nil && tried[uid] {
			continue
