...
```bash
# 国内模型列表（正式前缀 `/cn/v1`）
curl -s http://localhost:7863/cn/v1/models -H "Authorization: Bearer your-api-key"

# 国际模型列表（正式前缀 `/intl/v1`）
curl -s http://localhost:7863/intl/v1/models -H "Authorization: Bearer your-api-key"

# 国内站流式聊天
curl -sN http://localhost:7863/cn/v1/chat/completions \
  -H "Authorization: Bearer your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"hi"}],"stream":true}'

# 国际站非流式聊天
curl -s http://localhost:7863/intl/v1/chat/completions \
  -H "Authorization: Bearer your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"model":"deepseek-v4.1-flash","messages":[{"role":"user","content":"hi"}],"stream":false}'

# 健康检查
curl -s http://localhost:7863/cn/healthz
curl -s http://localhost:7863/intl/healthz

```

OpenAI SDK 的 `base_url` 分别设置为：

```text
国内：http://服务器:7863/cn/v1
国际：http://服务器:7863/intl/v1
```

两站路由严格隔离：国内路径只选择 `site=cn` 账号，国际路径只选择 `site=intl` 账号；
同站无可用账号时返回 503，不会跨站回退。旧路径 `/v1/chat/completions`、`/v1/cn/chat/completions`、
`/v1/intl/chat/completions`、`/v1/models`、`/v1/cn/models`、`/v1/intl/models` 继续保留兼容，
其中无前缀站点的旧 `/v1/*` 固定等价于国内站。
...
| `features.default_1m_context` | `true` | 支持 1M 的模型默认选择 1M 会话预算；关闭后默认 200K，仅修改模型元数据，不改写请求 `max_tokens` |
...
| **模型与档位** | 实时查询上游：每模型的积分倍率、默认思考档、支持的档位（含「off（可关）」）、上下文长度与最大输出；支持 1M 的模型可通过配置开关切换默认 1M / 200K |
| **配置** | 在线编辑 config.json：API 密钥、定时任务（四类任务时点与开关、余额刷新间隔）、账号池与流量治理参数、上游超时与 UA、提示词模式、默认 1M/脱敏/粘性开关 |
| **运行日志** | 最近 500 行服务日志 + 请求表格日志（可开关自动滚动） |

**配置热生效**：保存配置后，`api_key`、`cooldown.soft_rate`、`features.sanitize_blacklist_fingerprints`、
`features.default_1m_context`、`pool.*`（熔断/在途/权重）、`schedule.*`（时点/开关/余额刷新间隔）**立即生效，无需重启**；
...
正式 API 前缀将站点放在 `/v1` 前，适合作为 OpenAI SDK 的 `base_url`：

| 端点 | 鉴权 | 说明 |
|---|---|---|
| `POST /cn/v1/chat/completions` | Bearer（`api_key` 非空时） | 国内版 OpenAI 兼容补全，仅使用国内账号 |
| `GET /cn/v1/models` | Bearer（`api_key` 非空时） | 国内模型列表 |
| `POST /intl/v1/chat/completions` | Bearer（`api_key` 非空时） | 国际版 OpenAI 兼容补全，仅使用国际账号 |
| `GET /intl/v1/models` | Bearer（`api_key` 非空时） | 国际模型列表 |
| `GET /cn/healthz` / `GET /intl/healthz` | 无 | 国内/国际站独立健康检查 |
| `POST /v1/cn/chat/completions` / `POST /v1/intl/chat/completions` | Bearer（`api_key` 非空时） | 旧站点路径兼容入口 |
| `GET /v1/cn/models` / `GET /v1/intl/models` | Bearer（`api_key` 非空时） | 旧站点模型路径兼容入口 |
| `POST /v1/chat/completions` | Bearer（`api_key` 非空时） | 旧兼容入口，固定等价于国内版 |
| `GET /v1/models` | Bearer（`api_key` 非空时） | 旧兼容入口，固定返回国内模型列表 |
| `GET /status` | Bearer（`api_key` 非空时） | 账号状态汇总 + 每账号详情（积分/冷却/熔断/在途/粘性） |
| `GET /healthz` | 无 | 全池健康检查 |
| `GET /healthz/cn` / `GET /healthz/intl` | 无 | 旧国内/国际健康检查兼容入口 |

国内 OpenAI SDK 的 `base_url` 使用 `http://服务器:7863/cn/v1`，国际 SDK 使用 `http://服务器:7863/intl/v1`。
路径决定站点，模型名不会自动跨站切换；同站无可用账号时返回 503，不会跨站回退。
